...
import { z } from "zod";
import { generateObjectFn } from "@/app/api/chat/action";
...
export async function POST(request: Request) {
  try {
    ...
    const intel = analyzeProject(String(body?.name || "project"), files);
    const prompt = `وظیفه: ${objective}\n\n${buildContextPack(intel)}\n\nفقط تغییرات لازم را پیشنهاد بده. مسیرها نسبی به ریشه پروژه باشند. secret تولید نکن. پاسخ باید JSON مطابق schema باشد.`;
    const result = await generateObjectFn([{ id: crypto.randomUUID(), role: "user", content: prompt }]);
