...
function resolveModel(modelKey: string) {
  ...
  if (LEGACY_FREE_FALLBACKS.has(modelKey)) return { provider: "openrouter", model: "openrouter/free", apiKey: process.env.OPENROUTER_API_KEY, apiUrl: "https://openrouter.ai/api/v1" };
...
export function getOpenAIModel(baseURL: string, apiKey: string, model: string) {
  ...
  if (provider === "deepseek") { initOptions = {}; return createDeepSeek({ apiKey: key, baseURL: url })(resolved.model); }
  if (provider === "zhipu") { initOptions = { maxTokens: 65536 }; return createOpenAI({ apiKey: key, baseURL: url, compatibility: "compatible" })(resolved.model); }
  if (provider === "openrouter") { initOptions = {}; return createOpenAI({ apiKey: key, baseURL: url, compatibility: "compatible", headers: { "HTTP-Referer": process.env.APP_BASE_URL || "http://localhost:3000", "X-Title": "We0 Free AI" } })(resolved.model); }
  initOptions = {};
