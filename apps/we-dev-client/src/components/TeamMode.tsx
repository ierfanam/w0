...
type Result = { ok?: boolean; error?: string; plan?: any; runtimePlan?: any; results?: any[]; final?: any; adversarial?: any; qualityBefore?: any };
...
export default function TeamMode({ apiBase = "" }: Props) {
  ...
  function downloadPlan() {
    if (!result?.runtimePlan) return;
    const blob = new Blob([JSON.stringify(result.runtimePlan, null, 2)], { type: "application/json" });
    const url = URL.createObjectURL(blob); const a = document.createElement("a"); a.href = url; a.download = "w0-autopilot-workflow.json"; a.click(); URL.revokeObjectURL(url);
  }
